...
each data entity - added in the workflow below as an additional line in the
call to `mutate.` Also, some parameters are different, such as unlike
`create_vector_shape`, which requires the user to pass an EML-compliant
coordinate reference system, since `create_vector_kml` writes to kml, the
resulting CRS is *always* EPSG 4326 and, thus, is hard-coded into the function.

```{r create-vector-kml, eval=FALSE}
